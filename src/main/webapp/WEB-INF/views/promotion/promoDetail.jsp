		...
		<div class="mb-4 text-center">
		    <c:choose>
				<c:when test="${not empty promo.promotionPageUrl}">
				    <a href="${promo.promotionPageUrl}" target="_blank" class="homepage-link">
				        홈페이지 바로가기
				    </a>
				</c:when>
		        <c:otherwise>
            ...
            if (deleteButton) {
                deleteButton.addEventListener('click', async function() {
                    ...
                    if (confirmed) {
                        ...
                        // URLSearchParams를 사용하여 데이터 구성 (promoId만 본문에 포함)
                        const params = new URLSearchParams();
                        params.append('promoId', '${promo.promoId}');
                        // params.append(csrfHeaderName, csrfToken); // 본문이 아닌 헤더에 추가하므로 이 줄은 제거합니다.

                        // fetch 요청: headers에 CSRF 토큰을 추가
                        fetch('${contextPath}/promoBoard/delete', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded',
                                [csrfHeaderName]: csrfToken // CSRF 토큰을 HTTP 헤더에 추가
                            },
                            body: params.toString() // promoId만 본문에 전송
                        })
                        .then(response => {
                            // 응답 상태가 200 OK가 아니면 오류 처리
                            if (!response.ok) {
                                // HTTP 에러 상태 코드에 따른 상세 에러 메시지 처리
                                if (response.status === 403) {
                                    // 403 Forbidden 오류 시: 응답이 JSON이 아닐 가능성이 높으므로 response.text()로 먼저 파싱
                                    return response.text().then(errorMessage => {
                                        console.error("403 Forbidden 응답 내용:", errorMessage);
                                        throw new Error('권한이 없거나 요청이 위조되었을 수 있습니다.');
                                    });
                                }
                                // 그 외 HTTP 오류: JSON 응답을 기대할 수 있으므로 json()으로 시도
                                return response.json().then(errorData => {
                                    throw new Error(errorData.message || `HTTP 오류! 상태: ${response.status}`);
                                }).catch(() => {
                                    // JSON 파싱 실패 시: 일반 텍스트로 처리하여 SyntaxError 방지
                                    return response.text().then(errorMessage => {
                        ...
                        .then(data => {
                            if (data.msg === 'success') {
                                window.showCommonModal("삭제 완료", "게시글이 성공적으로 삭제되었습니다.").then(() => {
                                    window.location.href = '${contextPath}/promoBoard'; // 성공 시 게시글 목록으로 이동
                                });
                            } else {
                                // 서버에서 'success'가 아닌 다른 메시지를 보낸 경우
