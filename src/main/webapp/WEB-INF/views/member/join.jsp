...
<body>
	<div class="main">
		...
		<h2>회원가입</h2>
		<form method="post"
			action="${pageContext.request.contextPath}/member/join" id="joinForm"
			...
			<div class="form-row">
				<div class="form-group">
					<label for="name">이름</label> <input name="userName" class="box3" 
						minlength="2" type="text" id="name" name="name" required/>
				</div>
				<div class="form-group">
					<label for="birth">생년월일</label> <input name="userBrth" class="box3"
			...
			<div>
				...
				<input class="box2" name="nickname" required type="text"
					placeholder="닉네임입력"> <input class="but" type="button"
					value="중복확인" onclick="checkNick()"> <!-- 함수on -->
			</div>
			...
			<div>
				...
				<div>
					...
					<input id="address" name="addres1" required class="my-add" type="text"
	...
	<script>
      ...
      function checkId(){
      	...
      	let memberId = $('input[name=userId]').val(); //입력한 아이디값
      	
      	$.ajax({
      		url: '${pageContext.request.contextPath}/member/idChecker',			//컨트롤러
      		type: 'POST',						//GET,POST인지
      		async: true,
      		data: {
      			userId: memberId				//DATA필드
      		},
	...
	<script>
        ...
        let membernick = $('input[name=nickname]').val();
        ...
        function checkNick() {
			...
			$.ajax({
				url: `${pageContext.request.contextPath}/member/nickChecker`,
				type: 'POST',
